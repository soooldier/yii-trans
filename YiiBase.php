...
class YiiBase
{
	 ...
	 *
	 * Import一个类文件相当于直接include类文件，它们的主要区别在于import比include更
	 * lighter因为它只会在文件第一次被引用的时候真正include。
	 ...
	 *
	 * Import一个目录和把目录加入到inlude path是等效的。如果是多个目录被Import，
	 * 那么后import的目录会优先被搜索（后import的目录会被加到include path的前面）
	 ...
	 *
	 * 从1.1.5版本开始，这个方法也可以用来import命名空间格式的文件
	 * （PHP 5.3以上版本）
